...
>(({ className, variant, ...props }, ref) => {
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={cn("loupe-system", toastVariants({ variant }), className)}
      {...props}
    />
  );
});
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-1 top-1 rounded-md p-1 text-foreground/50 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-hidden focus:ring-1 group-hover:opacity-100 group-[.destructive]:text-destructive hover:group-[.destructive]:text-destructive focus:group-[.destructive]:ring-destructive focus:group-[.destructive]:ring-offset-destructive",
      className,
    )}
    toast-close=""
    {...props}
